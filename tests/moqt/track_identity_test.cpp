...
byte_buffer_view as_view(const byte_buffer& owned_bytes) {
    return owned_bytes;
}

TEST(track_namespace_test, accepts_empty_namespace_prefix) {
    constexpr std::array<byte_buffer_view, 0> fields = {};
...
TEST(track_namespace_test, preserves_binary_fields) {
    std::vector first = {std::byte{0x00}, std::byte{0x7f}, std::byte{0xff}};
    std::vector second = {std::byte{0x12}};
    ...
    EXPECT_EQ(name_space->fields()[0][0], std::byte{0x00});
    EXPECT_EQ(name_space->fields()[0][1], std::byte{0x7f});
    EXPECT_EQ(name_space->fields()[0][2], std::byte{0xff});
    EXPECT_EQ(name_space->fields()[1][0], std::byte{0x12});
...
TEST(track_namespace_test, owns_copied_field_bytes) {
    ...
    source[0] = std::byte{0x20};

    ASSERT_TRUE(ns.has_value());
    EXPECT_EQ(ns->fields()[0][0], std::byte{0x10}); // copy so not equivalent anymore
}

TEST(track_namespace_test, rejects_empty_namespace_field) {
    std::array fields = {byte_buffer_view()};
...
TEST(track_namespace_test, accepts_thirty_two_namespace_fields) {
    constexpr std::array<std::byte, 1> field = {std::byte{0x01}};
    std::array<byte_buffer_view, track_namespace::max_field_count> fields{};
    fields.fill(byte_buffer_view(field));
...
TEST(track_namespace_test, rejects_thirty_three_namespace_fields) {
    constexpr std::array field = {std::byte{0x01}};
    std::array<byte_buffer_view, track_namespace::max_field_count + 1> fields{};
    fields.fill(byte_buffer_view(field));
...
TEST(track_namespace_test, accepts_four_kib_namespace) {
    const std::vector field(track_namespace::max_byte_count, std::byte{0x01});
...
TEST(track_namespace_test, rejects_namespace_over_four_kib) {
    const std::vector field(track_namespace::max_byte_count + 1, std::byte{0x01});
    const std::array fields = {as_view(field)};
...
TEST(track_name_test, accepts_empty_track_name) {
    const auto name = track_name::make(byte_buffer_view());
...
TEST(track_name_test, owns_copied_bytes) {
    ...
    const auto name = track_name::make(byte_buffer_view(source));
    source[0] = std::byte{0x20};

    ASSERT_TRUE(name.has_value());
    EXPECT_EQ(name->bytes()[0], std::byte{0x10});
}

TEST(track_name_test, equality_and_ordering_use_exact_bytes) {
    constexpr std::array low_bytes = {std::byte{0x01}};
    constexpr std::array high_bytes = {std::byte{0x02}};

    const auto low_name = track_name::make(byte_buffer_view(low_bytes));
    const auto same_low_name = track_name::make(byte_buffer_view(low_bytes));
    const auto high_name = track_name::make(byte_buffer_view(high_bytes));
...
TEST(track_namespace_test, equality_and_ordering_use_exact_bytes) {
    ...
    const std::array low_fields = {byte_buffer_view(low)};
    const std::array high_fields = {byte_buffer_view(high)};
...
TEST(full_track_name_test, exposes_namespace_name_and_byte_count) {
    constexpr std::array field = {std::byte{0x01}};
    constexpr std::array name_bytes = {std::byte{0x02}, std::byte{0x03}};
    const std::array fields = {byte_buffer_view(field)};
    const auto name_space = track_namespace::make(fields);
...
TEST(full_track_name_test, accepts_four_kib_full_track_name) {
    const std::vector name_bytes(full_track_name::max_byte_count, std::byte{0x01});
    const std::array<byte_buffer_view, 0> fields = {};
    const auto name_space = track_namespace::make(fields);
...
TEST(full_track_name_test, rejects_full_track_name_over_four_kib) {
    constexpr std::array field = {std::byte{0x01}};
    const std::vector name_bytes(full_track_name::max_byte_count, std::byte{0x02});
    const std::array fields = {byte_buffer_view(field)};
    const auto name_space = track_namespace::make(fields);
    const auto name = track_name::make(as_view(name_bytes));
...
TEST(full_track_name_test, equality_and_ordering_use_namespace_then_name_bytes) {
    constexpr std::array namespace_bytes = {std::byte{0x01}};
    constexpr std::array low_name_bytes = {std::byte{0x01}};
    constexpr std::array high_name_bytes = {std::byte{0x02}};
    const std::array fields = {byte_buffer_view(namespace_bytes)};

    const auto name_space = track_namespace::make(fields);
    const auto low_name = track_name::make(byte_buffer_view(low_name_bytes));
    const auto same_low_name = track_name::make(byte_buffer_view(low_name_bytes));
    const auto high_name = track_name::make(byte_buffer_view(high_name_bytes));
